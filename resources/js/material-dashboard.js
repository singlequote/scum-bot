import PerfectScrollbar from './plugins/perfect-scrollbar.min.js';
//import SmoothScrollbar from './plugins/smooth-scrollbar.min.js';
import bootstrap from './core/bootstrap.bundle.min';

require('./plugins/choices.min.js');
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    $('.js-choice').each((index, el) => {
        new Choices(el);
    });
    $('.js-zero').each((index, el) => {
        $(el).before(`<input type="hidden" value="0" name="${$(el).attr('name')}"/>`);
    });
    

    $('.input-group-outline input').each((index, input) => {
        if ($(input).val().length) {
...
window.sidebarColor = (a) => {
    ...
    $.post(route('user-settings.store'), {_token: csrfToken(), _method: 'post', sidebar_color: color});
...
window.sidebarType = (a) => {
    ...
    $.post(route('user-settings.store'), {_token: csrfToken(), _method: 'post', sidebar_type: color});
...
window.navbarFixed = (el) => {
    ...
    $.post(route('user-settings.store'), {_token: csrfToken(), _method: 'post', fixed_navbar: $(el).is(':checked') ? 1 : 0});
...
/*
 ...
 |
 */
window.navbarMinimize = (el) => {
    var sidenavShow = document.getElementsByClassName('g-sidenav-show')[0];

    $.post(route('user-settings.store'), {_token: csrfToken(), _method: 'post', sidebar_collapse: $(el).is(':checked') ? 1 : 0});
...
//    let toggleNavbarMinimize = document.getElementById('navbarMinimize');
    ...
    if (sidenavShow) {
        sidenavToggler.onclick = function () {
            ...
            $('#navbarMinimize').prop("checked", !$('#navbarMinimize').prop("checked"));

            navbarMinimize($('#navbarMinimize')[0]);
//            $('#navbarMinimize').toggle('click');
...
window.darkMode = (el) => {
    ...
    $.post(route('user-settings.store'), {_token: csrfToken(), _method: 'post', darkmode: $(el).is(':checked') ? 1 : 0});
